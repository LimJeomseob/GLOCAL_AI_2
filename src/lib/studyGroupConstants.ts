...
export const STUDY_PLAN_SECTIONS: StudyPlanSectionDef[] = [
 ...
 ○ 결과 활용방안
  ...
  },
  {
    ...
    hint: "아래 워크숍 희망일·시작 시간 표를 함께 작성해 주세요. 표에 적은 날짜와 시간으로 강사 배정을 검토합니다.",
    scoreNote: "심사기준 4번 '구체적인 요청사항' 10점",
    example: `가. 워크숍 개설 요청 일자
 ○ 단계별 워크숍 개설 요청 (시작 시간 포함, 단계별 3시간)
  - 1안: 1차 기획(9월 29일 14:00), 2차 제작(10월 13일 14:00), 3차 환류(10월 27일 14:00)
  - 2안: 1차 기획(9월 30일 10:00), 2차 제작(10월 14일 10:00), 3차 환류(10월 28일 10:00)`,
  },
];
...
// 아래 문구는 공문 원문이다. 임의로 다듬지 말 것.
//
// 신청기간은 공문 원문대로 9. 9.(수) ~ 9. 18.(금)이다. 0020에서 접수를 9. 7.로 이틀
// 앞당겼다가 공문 일자로 되돌렸다(0022).
// 실제 접수 허용 구간의 정본은 study_rounds.expert_apply_open_at/close_at이며,
// 이 표기는 화면 안내일 뿐이다 — 둘을 함께 바꿔야 안내와 동작이 어긋나지 않는다.
// ----------------------------------------------------------------------------

export const STUDY_EXPERT_TITLE = "교내 AI활용 전문가 신청";

/** 공문 2항 본문 */
export const STUDY_EXPERT_INTRO =
  "「2026학년도 AI활용 연구모임」 참여 팀의 연구과제 해결을 지원할 교내 AI활용 전문가를 모집합니다.";

/** 공문 가~바·아 항목. 화면 요약 카드에 그대로 표시한다. */
export const STUDY_EXPERT_OVERVIEW = [
  { label: "역할", value: "연구모임별 맞춤형 교육 및 코칭 3회(기획, 제작, 환류)" },
  { label: "모집대상", value: "생성형 AI 활용 교수법 또는 연구 경험이 있는 본교 교원(비전임교원 포함)" },
  { label: "모집인원", value: "5명 내외" },
  { label: "신청기간", value: "´26. 9. 9.(수) ~ 9. 18.(금)" },
  { label: "운영기간", value: "´26. 9. 28.(월) ~ 11. 13.(금) 중 3회" },
  { label: "지원사항", value: "소정의 강사료 및 유료계정 지원" },
] as const;

/** 공문 사항 활동내용 3단계 */
export const STUDY_EXPERT_ACTIVITIES = [
  { step: "기획", detail: "연구모임별 연구과제 해결을 위한 기획 및 AI도구 사용법 안내" },
  { step: "제작", detail: "강의도구 및 콘텐츠 MVP 제작" },
  { step: "환류", detail: "제작시 발생한 오류 해결 및 배포" },
] as const;

export const STUDY_EXPERT_HEADCOUNT = 5;
...
export const STUDY_EXPERT_CONSENT_ITEMS = [
  ...
  "수집 목적: 교내 AI활용 전문가 신청 접수, 선정, 연구모임 배정 및 운영 안내, 강사료·유료계정 지원",
  "보유 기간: 사업 종료 후 5년 또는 관계 법령에 따름",
  "동의를 거부하실 수 있으나, 거부 시 전문가 신청이 불가합니다.",
] as const;

/** 제출 확인 모달에 노출하는 서명 문구 */
export const STUDY_EXPERT_SIGNATURE =
  "위와 같이 AI 활용 연구모임 교내 AI활용 전문가로 신청합니다.";
